...
{{-- Hero Section --}}
<section class="relative h-[60vh] bg-cover bg-center flex items-center justify-center text-center text-white" style="background-image: linear-gradient(rgba(10, 28, 17, 0.7), rgba(10, 28, 17, 0.8)), url('{{ asset('gambar/gambar-produk.jpeg') }}');">
            ...
            <div id="category-grid" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                @forelse ($categories as $index => $category)
                    <div class="category-card-container reveal-animation" style="transition-delay: {{ $index * 50 }}ms;">
                        <div class="category-card cursor-pointer group relative rounded-lg overflow-hidden shadow-lg" data-category-id="{{ $category->id }}" data-category-name="{{ $category->category_name }}">
                            <img src="{{ asset('storage/' . $category->image_url) }}" alt="{{ $category->category_name }}" class="w-full h-80 object-cover" onerror="this.src='https://placehold.co/500x500/102b19/E6D793?text={{ urlencode($category->category_name) }}'">
                            <div class="absolute inset-0 bg-gradient-to-t from-primary/80 to-transparent"></div>
                            <h3 class="absolute bottom-6 left-6 text-2xl font-serif font-bold text-white">{{ $category->category_name }}</h3>
                        </div>
                    </div>
                @empty
