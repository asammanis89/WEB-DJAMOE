...
<section class="relative h-[60vh] bg-cover bg-center flex items-center justify-center text-center text-white"
    style="background-image: linear-gradient(rgba(10, 28, 17, 0.7), rgba(10, 28, 17, 0.8)), url('{{ asset('gambar/gambar2.jpeg') }}');">
...
<section class="py-24">
    <div class="container mx-auto px-4 space-y-16 md:space-y-24">
        ...
        <div class="flex flex-col {{ $loop->even ? 'md:flex-row-reverse' : 'md:flex-row' }} items-center gap-8 md:gap-16 reveal-animation">
            ...
            <!-- Teks -->
            {{-- DIPERBAIKI: Mengubah perataan teks menjadi text-center di mobile --}}
            <div class="md:w-1/2 text-center {{ $loop->even ? 'md:text-right' : 'md:text-left' }}">
                ...
                <h3 class="text-4xl md:text-5xl font-serif font-bold mt-2 text-accent">{{ $item->title }}</h3>
                {{-- DIPERBAIKI: Mengubah posisi garis pemisah menjadi di tengah pada mobile --}}
